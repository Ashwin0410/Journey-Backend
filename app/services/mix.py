...
#         vf = "highpass=f=80, equalizer=f=3800:t=h:w=2:g=-1.5"
#         try:
#             subprocess.run(
...
#         voice_exact = voice
#         frame_bytes = ch * sw
#         target_samples_per_ch = len(music_exact.raw_data) // frame_bytes
#         target_ms = int(round(1000 * target_samples_per_ch / music_exact.frame_rate))
#     else:
        
#         t_ms = target_ms
#         voice_exact = _hard_fit(voice, t_ms)
#         music_exact = music
...
#         # lookahead region for voice energy
#         start_v_la = i_ms + lookahead_ms
#         end_v_la = start_v_la + win
#         v_chunk_la = voice[start_v_la:end_v_la]
#         v_db_la = _rms_dbfs(v_chunk_la)

#         # current frame voice activity
#         v_now = voice[i_ms: i_ms + win]
#         v_now_db = _rms_dbfs(v_now)
#         voice_now = v_now_db > silence_threshold_db
...
#         # map voice level (lookahead) to target gain
#         if v_db_la <= -48.0:
#             target = floor_boost_db
#         elif v_db_la >= -26.0:
#             target = max_duck_db
...
#         # hold level slightly during short gaps in speech
#         short_gap = (not voice_now) and in_voice_region and (silence_run_ms < gap_hold_ms)
#         if short_gap:
#             hold_level = max(max_duck_db + 1.5, -1.5)
#             target = min(target, hold_level)
...
#     # Previously: bass boost ("bass=g=5:f=100").
#     # Now: tighten low-end and reduce boom around 50–80 Hz.
#     if _ffmpeg_has(ffmpeg_path, "equalizer"):
#         tmp_m_in = tempfile.NamedTemporaryFile(delete=False, suffix=".wav")
#         tmp_m_out = tempfile.NamedTemporaryFile(delete=False, suffix=".wav")
#         music.export(tmp_m_in.name, format="wav")
#         try:
#             # Cut a bit at 50 Hz and 80 Hz to tame overpowering bass.
...
#             music = AudioSegment.from_file(tmp_m_out.name)
#         except Exception:
#             pass  # fallback to un-EQ'd music if anything fails

#     # Gentle compression / leveling on music
#     if _ffmpeg_has(ffmpeg_path, "acompressor") or _ffmpeg_has(ffmpeg_path, "dynaudnorm"):
#         tmp_m2_in = tempfile.NamedTemporaryFile(delete=False, suffix=".wav")
#         tmp_m2_out = tempfile.NamedTemporaryFile(delete=False, suffix=".wav")
#         music.export(tmp_m2_in.name, format="wav")
...
#     # Voice EQ: keep highpass, add low-end warmth, keep slight high cut.
#     if _ffmpeg_has(ffmpeg_path, "equalizer") or _ffmpeg_has(ffmpeg_path, "highpass"):
#         tmp_v_in = tempfile.NamedTemporaryFile(delete=False, suffix=".wav")
#         tmp_v_out = tempfile.NamedTemporaryFile(delete=False, suffix=".wav")
#         voice.export(tmp_v_in.name, format="wav")
#         # Slightly lower HPF, add low-end boost around 150 Hz for richness,
#         # and keep a small cut around 3.8 kHz to tame harshness.
...
#     # --- DUCKING ---
#     # Make ducking less aggressive so the music keeps more emotional impact under VO.
#     music_adapt = _duck_music_to_voice(
#         music_exact,
#         voice_exact,
#         floor_boost_db=3.0,   # keep some lift between phrases
#         max_duck_db=-1.5,     # was -3.0 -> now shallower duck = louder under VO
#         attack_ms=180,
#         release_ms=650,
#         win_ms=60,
#         lookahead_ms=500,
...
def mix(
    ...
    **_ignored,
    ...
    # Base target from music (can be overridden below)
    target_samples_per_ch = len(music.raw_data) // frame_bytes
    target_ms = int(round(1000 * target_samples_per_ch / music.frame_rate))
    ...
    if sync_mode == "retime_voice_to_music":
        ...
        # "no_retime_trim_pad":
        # Do NOT time-stretch either stem.
        # Instead, hard-fit BOTH music and voice to the VOICE duration so that
        # speech and music end together. This matches Felix's "when music stops,
        # speech should conclude" requirement.
        voice_frames = len(voice.raw_data) // frame_bytes
        target_samples_per_ch = voice_frames
        target_ms = int(round(1000 * target_samples_per_ch / voice.frame_rate))
    ...
    # --- DUCKING ---
    # Make ducking less aggressive so the music keeps emotional impact,
    # but with slightly louder VO (handled via voice_target_dbfs).
    music_adapt = _duck_music_to_voice(
        music_exact,
        voice_exact,
        floor_boost_db=3.0,   # keep some lift between phrases
        max_duck_db=-1.5,     # shallower duck = music still feels present
        attack_ms=180,
        release_ms=650,
        win_ms=60,
        lookahead_ms=500,
